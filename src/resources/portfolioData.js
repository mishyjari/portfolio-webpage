...
The back-end is built with Ruby on Rails and the front-end is a React application. Styling is handled mostly through React-Bootstrap, with some custom CSS worked in.`,
      ...
      },
      {
        ...
        longDescription: `Punchr’s main page will render a punch form, a list of all active shifts, and a visual overview of active shifts as a graph. If a PIN is entered which matches a user, that user’s information will be rendered. If the user has an active shift, the details of that shift will also be displayed. Submitting the punch will open or close the punch as appropriate.
Opening the control panel will first render a login form. Once logged in, a non-manager user will be able to view a list of all employees, view or edit their information, and view a history of their previous shifts. Managers will have the added ability to create, edit or delete users, see histories of all shifts, and edit or delete shifts.
Punchr is built on a Rails API with a React front-end.`,
      ...
      },
      {
        ...
        longDescription: `Upon loading, ArrowJournal will allow a user to login or create a new account. Event and Task instances are relational to each user, so a user account must exist for the journal to be used. There are several font choices available, which can be selected via the menu at the top of the page. Upon logging in, ArrowJournal will present a greeting and a list of events and tasks for today.
Hovering the cursor over a task or event will render a modal on the opposite page with the details for that item, as well as the ability to edit it. Events will render blue, tasks marked ‘important’ will render red, normal tasks will render green, and completed tasks will render grey. A task’s completion status can be toggled via alt-click.
